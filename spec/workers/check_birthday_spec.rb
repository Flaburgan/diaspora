...
describe CheckBirthdayWorker do
  ...
  before do
    ...
    allow(Notifications::ContactsBirthdayService).to receive(:notify)
  end

  it "calls notify method for the birthday person's contacts" do
    CheckBirthdayWorker.new.perform
    expect(Notifications::ContactsBirthdayService).to have_received(:notify).with(contact1)
    expect(Notifications::ContactsBirthdayService).to have_received(:notify).with(contact2)
  end

  it "does nothing if the birthday does not exist" do
    birthday_profile.update(birthday: nil)
    CheckBirthdayWorker.new.perform
    expect(Notifications::ContactsBirthdayService).not_to have_received(:notify).with(contact1)
    expect(Notifications::ContactsBirthdayService).not_to have_received(:notify).with(contact2)
  end
